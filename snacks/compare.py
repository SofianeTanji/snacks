...
def compare(dataset, nb_runs):
    ...
    run_snacks(Xtr, Ytr, Xts, Yts, 3, 1e-7, 1.0)

    # SNACKS
    tr_scores, ts_scores, times = [], [], []
    for i_run in range(nb_runs):
        print(f"Snacks : run {i_run + 1}/{nb_runs}")
        t_fit, tr_score, ts_score = run_snacks(
            Xtr, Ytr, Xts, Yts, 35000, values[dataset][1], 1.0
        )
    ...
    for i_run in range(1):
        ...
        t_fit, tr_score, ts_score = run_sklearn(
            oXtr, oYtr, oXts, oYts, values[dataset][0], values[dataset][1]
        )
        tr_scores.append(tr_score)
        ts_scores.append(ts_score)
        times.append(t_fit)
    ...
    for i_run in range(nb_runs):
        ...
        t_fit, tr_score, ts_score = run_pegasos(
            Xtr, Ytr, Xts, Yts, 300000, values[dataset][1]
        )
        tr_scores.append(tr_score)
        ts_scores.append(ts_score)
        times.append(t_fit)
    ...
    for i_run in range(nb_runs):
        ...
        t_fit, tr_score, ts_score = run_thundersvm(
            oXtr, oYtr, oXts, oYts, values[dataset][1]
        )
        tr_scores.append(tr_score)
        ts_scores.append(ts_score)
        times.append(t_fit)
